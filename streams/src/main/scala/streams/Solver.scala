...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val emptyList: LazyList[(Block, List[Move])] = LazyList()

    b.legalNeighbors.foldLeft(emptyList)({case (l, (block, move)) => (block, move :: history) #:: l})
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        (block, moves) <- initial
        (newBlock, newMove) <- block.legalNeighbors
        if !explored.contains(newBlock)
      } yield (newBlock, newMove :: moves)
